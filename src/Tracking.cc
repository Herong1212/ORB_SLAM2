...
namespace ORB_SLAM2
{
    ...
    void Tracking::Track()
    {
        ...
        else
        {
            ...
            if (bOK)
            {
                ...
                // Step 7：完成第二阶段跟踪后，要清除恒速模型跟踪中 UpdateLastFrame 中为当前帧临时添加的 MapPoints（仅双目和rgbd）
                // Delete temporal MapPoints
                // 步骤6中只是在当前帧中将这些 MapPoints 剔除，这里从 MapPoints 数据库中删除
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        // 若匹配地图点数目小于 15，则认为跟踪失败
        if (nmatches < 15)
            return false;

        // Step 3:将上一帧的位姿态，作为当前帧位姿的初始值，可以加速 BA 收敛
        mCurrentFrame.mvpMapPoints = vpMapPointMatches;
        mCurrentFrame.SetPose(mLastFrame.mTcw); // 用上一次的 Tcw 设置初值，在 PoseOptimization 可以收敛快一些
        ...
        for (int i = 0; i < mCurrentFrame.N; i++)
        {
            ...
            {
                // 如果对应到的某个特征点是外点（如果优化后判断某个地图点是外点，清除它的所有关系）
    ...
    void Tracking::UpdateLastFrame()
    {
        ...
        // Step 2：对于双目或 rgb-d 相机，为上一帧生成新的【临时地图点】
        ...
        // Step 2.2：从中找出不是地图点的点，并创建为【临时地图点】
        ...
        for (size_t j = 0; j < vDepthIdx.size(); j++)
        {
            ...
            // 如果这个点对应在上一帧中的地图点没有,或者创建后就没有被观测到,那么就生成一个【临时地图点】
            MapPoint *pMP = mLastFrame.mvpMapPoints[i];
            if (!pMP)
                bCreateNew = true;
            ...
            if (bCreateNew)
            {
                // Step 2.3：将需要创建的点，包装为地图点。只是为了提高双目和 RGB-D 的跟踪成功率
                // 并没有添加复杂属性，因为后面会扔掉。 反投影到世界坐标系中
                ...
                // 标记为临时添加的  MapPoint，之后在 CreateNewKeyFrame 之前会全部删除，并未添加新的观测信息
    ...
    {
        // 最小距离 【< 0.9* 次小距离】，就算匹配成功，检查旋转
        ORBmatcher matcher(0.9, true);

        // Step 1：更新上一帧的位姿；对于双目或 RGB-D 相机，还会根据深度值生成【临时地图点】
    ...
    bool Tracking::TrackLocalMap()
    {
        ...
        // Step 3：前面新增了更多的匹配关系，需要再一次进行 BA 优化，来得到更准确的位姿
        Optimizer::PoseOptimization(&mCurrentFrame);
        mnMatchesInliers = 0;

        // Step 4：更新当前帧的地图点被观测程度，并统计成功跟踪局部地图后匹配的总数目
        ...
        for (int i = 0; i < mCurrentFrame.N; i++)
        {
            if (mCurrentFrame.mvpMapPoints[i])
            {
                ...
                if (!mCurrentFrame.mvbOutlier[i])
                {
                    ...
                    if (!mbOnlyTracking)
                    {
                        // 如果该地图点被相机观测数目 nObs 大于0，匹配内点计数 +1
        ...
        // Step 5：根据成功跟踪匹配的总数目及重定位情况决定是否跟踪成功
        // Decide if the tracking was succesful
        // More restrictive if there was a relocalization recently
        // 如果最近刚刚发生了重定位，则跟踪成功的判定更严格，判定条件为——至少成功匹配 50 个点才认为是成功跟踪
        if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames && mnMatchesInliers < 50)
            return false;

        // 如果是正常的状态，只要成功跟踪到的地图点大于 30 个就认为跟踪成功了
    ...
    bool Tracking::NeedNewKeyFrame()
    {
        ...
        // mCurrentFrame.mnId 是当前帧的ID，mnLastRelocFrameId 是最近一次重定位帧的 ID
        ...
        // 双目或 RGB-D 情况下：成功跟踪到的地图点中近点太少 且没有跟踪到的近点（三维点）太多，可以插入关键帧了
        // 单目时，为 false
        bool bNeedToInsertClose = (nTrackedClose < 100) && (nNonTrackedClose > 70);

        // Step 7：决策是否需要插入关键帧
        // Step 7.1：设定比例阈值，当前帧和参考关键帧跟踪到点的比例，比例越大，越倾向于增加关键帧
        // 1. 双目相机或 RGB-D 相机模式下是 0.75
        float thRefRatio = 0.75f;

        // 2. 关键帧小于 2 帧时，插入关键帧的阈值设置的低一点，插入频率较低
        if (nKFs < 2)
            thRefRatio = 0.4f;
        ...
        // Step 7.2：条件 1a：很长时间没有插入关键帧，可以插入
        // Condition 1a: More than "MaxFrames" have passed from last keyframe insertion
        const bool c1a = mCurrentFrame.mnId >= mnLastKeyFrameId + mMaxFrames;

        // Step 7.3：条件 1b： 满足插入关键帧的最小间隔并且 localMapper 处于空闲状态，可以插入
        // Condition 1b: More than "MinFrames" have passed and Local Mapping is idle
        const bool c1b = (mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames && bLocalMappingIdle);

        // Step 7.4：条件 1c：在双目，RGB-D 的情况下当前帧跟踪到的点比参考关键帧的 0.25 倍还少，或者
        // 满足 bNeedToInsertClose（成功跟踪到的地图点中近点太少 且没有跟踪到的近点（三维点）太多）
        ...
        // Step 7.5：条件 2：和参考帧相比，当前帧跟踪到的点太少 或者满足 bNeedToInsertClose ；同时成功跟踪到的匹配内点数目还不能太少（大于 15）
        ...
        if ((c1a || c1b || c1c) && c2)
        {
            ...
            else
            {
                ...
                {
                    // 队列里不能阻塞太多关键帧
                    // tracking 插入关键帧不是直接插入，而且先插入到 mlNewKeyFrames 中，
    ...
    void Tracking::CreateNewKeyFrame()
    {
            ...
            if (!vDepthIdx.empty())
            {
                ...
                for (size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if (bCreateNew)
                    {
                        ...
                        // NOTE 这些添加属性的操作是每次创建 MapPoint 后都要做的！
        ...
        // Step 4：插入关键帧
        // 关键帧插入到列表 mlNewKeyFrames中，等待 local mapping 线程临幸
     ...
     */
    // todo 作用：筛选局部地图中新增的在视野范围内的地图点，投影到当前帧搜索匹配，得到更多的匹配关系👇
    void Tracking::SearchLocalPoints()
    {
        ...
        if (nToMatch > 0)
        {
            ...
            int th = 1;
            if (mSensor == System::RGBD) // RGBD 相机输入的时候,搜索的阈值会变得稍微大一些
                th = 3;
     ...
     */
    // todo 作用：红色点代表参考地图点，其实就是 Tracking 里的 local mappoints 👇
    ...
    void Tracking::UpdateLocalKeyFrames()
    {
            ...
            if (mCurrentFrame.mvpMapPoints[i])
            {
                ...
                if (!pMP->isBad())
                {
                    ...
                    for (map<KeyFrame *, size_t>::const_iterator it = observations.begin(), itend = observations.end(); it != itend; it++)
                        ...
                        // 所以最后 keyframeCounter 第 1 个参数表示某个关键帧，第 2 个参数表示该关键帧看到了多少当前帧 (mCurrentFrame) 的地图点，也就是共视程度
        ...
        // Step 2.1
        // todo 类型1：能观测到当前帧的地图点的关键帧作为局部关键帧 （将邻居拉拢入伙）（一级共视关键帧）
        ...
        for (map<KeyFrame *, int>::const_iterator it = keyframeCounter.begin(), itEnd = keyframeCounter.end(); it != itEnd; it++)
        {
            ...
            // 用该关键帧的成员变量 mnTrackReferenceForFrame 记录当前帧的 id
        ...
        for (vector<KeyFrame *>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end(); itKF != itEndKF; itKF++)
        {
            ...
            // todo 类型2:一级共视关键帧的共视（前 10 个）关键帧，称为二级共视关键帧（将邻居的邻居拉拢入伙）
            ...
            for (vector<KeyFrame *>::const_iterator itNeighKF = vNeighs.begin(), itEndNeighKF = vNeighs.end(); itNeighKF != itEndNeighKF; itNeighKF++)
            {
                ...
                if (!pNeighKF->isBad())
                {
                    // mnTrackReferenceForFrame 前面纪录的 ID 在这里使用，防止重复添加局部关键帧
            ...
            // todo 类型3:将一级共视关键帧的子关键帧作为局部关键帧（将邻居的孩子们拉拢入伙）
            ...
            // todo 类型3:将一级共视关键帧的父关键帧作为局部关键帧（将邻居的父母们拉拢入伙）
    ...
    bool Tracking::Relocalization()
    {
        ...
        // Step 3：遍历所有的候选关键帧，通过词袋进行快速匹配，用匹配结果初始化 PnP Solver
        for (int i = 0; i < nKFs; i++)
        {
            ...
            {
                // 当前帧和候选关键帧用 BoW 进行快速匹配，匹配结果记录在 vvpMapPointMatches，nmatches 表示匹配的数目
                int nmatches = matcher.SearchByBoW(pKF, mCurrentFrame, vvpMapPointMatches[i]);
                // 如果和当前帧的匹配数【小于 15】,那么只能放弃这个关键帧
                if (nmatches < 15)
                {
                    vbDiscarded[i] = true;
                    continue;
                ...
                {
                    // 如果匹配数目够用，用匹配结果初始化 EPnPsolver
                    // 为什么用 EPnP? 因为计算复杂度低，精度高
                    PnPsolver *pSolver = new PnPsolver(mCurrentFrame, vvpMapPointMatches[i]);
                    pSolver->SetRansacParameters(
                        0.99,   // 用于计算 RANSAC 迭代次数理论值的概率
                        10,     // 最小内点数, 但是要注意在程序中实际上是 min(给定最小内点数,最小集,内点数理论值),不一定使用这个
                        300,    // 最大迭代次数
                        4,      // 最小集(求解这个问题在一次采样中所需要采样的最少的点的个数,对于 Sim3 是 3,EPnP 是 4 ),参与到最小内点数的确定过程中
                        0.5,    // 这个是表示(最小内点数/样本总数);实际上的 RANSAC 正常退出的时候所需要的最小内点数其实是根据这个量来计算得到的
                        5.991); // 自由度为 2 的卡方检验的阈值,程序中还会根据特征点所在的图层对这个阈值进行缩放
        ...
        // 这里的 P4P RANSAC 是 Epnp，每次迭代需要 4 个点
        ...
        while (nCandidates > 0 && !bMatch)
        {
            ...
            for (int i = 0; i < nKFs; i++)
            {
                ...
                bool bNoMore; // 表示 RANSAC 已经没有更多的迭代次数可用 -- 也就是说数据不够好，RANSAC 也已经尽力了。。。

                // Step 4.1：通过 EPnP 算法估计姿态，迭代 5 次
                ...
                if (!Tcw.empty())
                {
                    //  Step 4.2：如果 EPnP 计算出了位姿，对内点进行 BA 优化
                    Tcw.copyTo(mCurrentFrame.mTcw);

                    // EPnP 里用 RANSAC 算法计算后的内点的集合
                    set<MapPoint *> sFound;
                    ...
                    if (nGood < 50)
                    {
                        // 通过投影的方式将关键帧中未匹配的地图点投影到当前帧中, 生成新的匹配关系
                        int nadditional = matcher2.SearchByProjection(
                            mCurrentFrame,     // 当前帧
                            vpCandidateKFs[i], // 关键帧
                            sFound,            // 已经找到的地图点集合，不会用于 PNP
                            10,                // 窗口阈值，会乘以金字塔尺度
                            100);              // 匹配的 ORB 描述子距离应该【小于】这个阈值
                        ...
                        if (nadditional + nGood >= 50)
                        {
                            ...
                            // Step 4.4：如果 BA 后内点数还是比较少 (<50 个)但是还不至于太少 (>30 个)，可以挽救一下, 最后垂死挣扎
                            ...
                            if (nGood > 30 && nGood < 50)
                            {
                                ...
                                // 如果成功挽救回来，匹配数目达到要求，最后进行一次 BA 优化
        ...
        else
        {
            ...
            return true; // 如果匹配上了,说明当前帧重定位成功了(当前帧已经有了自己的位姿)
        }
    }
