...
namespace ORB_SLAM2
{
     ...
     * @param[in] pbStopFlag            外部控制 BA 结束标志
     * @param[in] nLoopKF               形成了闭环的当前关键帧的 id
     * @param[in] bRobust               是否使用核函数
     */
    // todo 作用：BA 优化过程，由全局 BA 调用
                                     ...
                                     int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
    {
        ...
        // 如果这个时候外部请求终止，那就结束。注意，这句执行之后，外部再请求结束 BA，就结束不了了
        if (pbStopFlag)
            optimizer.setForceStopFlag(pbStopFlag);

        // 记录添加到优化器中的顶点的最大关键帧 id
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            // 对于每一个能用的关键帧构造 SE3 顶点,其实就是当前关键帧的位姿
            g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
            vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));
            // 顶点的 id 就是关键帧在所有关键帧中的 id
            vSE3->setId(pKF->mnId);
            // 只有第0帧关键帧不优化（参考基准）
     ...
     * 只优化 Frame 的 Tcw，不优化 MapPoints 的坐标
     *
     * 1. Vertex: g2o::VertexSE3Expmap()，即当前帧的 Tcw
     * 2. Edge:
     ...
     * @return  inliers数量
     */
    // todo 1 作用：主要用于 Tracking 线程中，1、2 阶段都有应用。仅优化位姿，不优化地图点。
    int Optimizer::PoseOptimization(Frame *pFrame)
    {
        // 该优化函数主要用于跟踪线程中：运动跟踪、参考帧跟踪、地图跟踪和重定位。

        // Step 1：构造 g2o 优化器, BlockSolver_6_3 表示：位姿 _PoseDim 为 6 维，路标点 _LandmarkDim 是 3 维
        // 第 1 步：创建线性求解器（ linearSolver ）
        // g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        // linearSolver = new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver = new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();

        // 第 2 步：创建块求解器 （ BlockSolver ），并用上面定义的线性求解器初始化
        g2o::BlockSolver_6_3 *solver_ptr = new g2o::BlockSolver_6_3(linearSolver);

        // 第 3 步：创建总求解器（ Solver ），并从 GN、LM、DogLeg 中选择一个，再用上面定义的块求解器初始化
        g2o::OptimizationAlgorithmLevenberg *solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);

        // 第 4 步：创建稀疏优化器（ SparseOptimizer ）
        ...
        // 自由度为 3 的卡方分布，显著性水平为 0.05，对应的临界阈值 7.815
        const float deltaStereo = sqrt(7.815);

        // Step 3：添加一元边
        {
            // 锁定地图点。由于需要使用地图点来构造顶点和边，因此不希望在构造的过程中部分地图点被改写，造成不一致，甚至是段错误
        ...
        // Step 4：开始优化，总共优化【4】次，每次优化迭代【10】次,每次优化后，将观测分为 outlier 和 inlier，outlier 不参与下次优化
        // 由于每次优化后是对所有的观测进行 outlier 和 inlier 判别，因此之前被判别为 outlier 有可能变成 inlier，反之亦然
        // 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）
        const float chi2Mono[4] = {5.991, 5.991, 5.991, 5.991};   // 单目
        const float chi2Stereo[4] = {7.815, 7.815, 7.815, 7.815}; // 双目
        const int its[4] = {10, 10, 10, 10};                      // 4 次迭代，每次迭代的次数
        ...
        for (size_t it = 0; it < 4; it++)
        {
            ...
            vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
            // 其实就是初始化优化器,这里的参数 0 就算是不填写,默认也是 0,也就是只对 level 为 0 的边进行优化
            optimizer.initializeOptimization(0);
            // 开始优化，优化【10】次
            optimizer.optimize(its[it]);

            nBad = 0;
            // 优化结束,开始遍历参与优化的每一条误差边(单目相机模式)
            for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
            {
                ...
                // 就是 error*\Omega*error, 表征了这个点的误差大小(考虑置信度以后)
                const float chi2 = e->chi2();

                if (chi2 > chi2Mono[it])
                {
                    pFrame->mvbOutlier[idx] = true;
                    e->setLevel(1); // 设置为 outlier , level 1 对应为外点, 上面的过程中我们设置其为【不优化】
                    nBad++;
                }
                else
                {
                    pFrame->mvbOutlier[idx] = false;
                    e->setLevel(0); // 设置为 inlier, level 0 对应为内点, 上面的过程中我们就是【要优化】这些关系
                }

                if (it == 2)
                    e->setRobustKernel(0); // 除了前两次优化需要使用鲁棒和核函数以外, 其余的优化都不需要 -- 因为重投影的误差已经有明显的下降了
            } // 对单目误差边的处理

            // 双目相机或 RGB-D 相机：同样的原理，遍历双目的误差边和单目相机一样
     ...
     */
    // todo 2 作用：主要用于优化局部建图中的局部关键帧和局部地图点。
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap)
    {
        // 该优化函数用于 LocalMapping 线程的局部 BA 优化

        // 局部关键帧: First Breadth Search from Current Keyframe
        ...
        for (int i = 0, iend = vNeighKFs.size(); i < iend; i++)
        {
            ...
            // 把参与局部 BA 的每一个关键帧的 mnBALocalForKF 设置为当前关键帧的 mnId，防止重复添加
            ...
            for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend; vit++)
            {
                ...
                if (pMP)
                {
                    if (!pMP->isBad()) // 保证地图点有效
                        // 把参与局部BA的每一个地图点的 mnBALocalForKF 设置为当前关键帧的 mnId
        ...
        // Step 3 得到能被局部地图点观测到，但不属于局部关键帧的关键帧(二级相连)，这些二级相连关键帧在局部 BA 优化时不优化
        // Fixed Keyframes. Keyframes that see Local MapPoints but that are not Local Keyframes
        list<KeyFrame *> lFixedCameras;
        // 遍历局部地图中的每个地图点
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
        {
            // 观测到该地图点的 KF 和该地图点在 KF 中的索引
            ...
            for (map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
            {
                ...
                // pKFi->mnBAFixedForKF!=pKF->mnId 表示还未标记为 fixed（固定的）关键帧
                if (pKFi->mnBALocalForKF != pKF->mnId && pKFi->mnBAFixedForKF != pKF->mnId)
                {
                    // 将局部地图点能观测到的、但是不属于局部 BA 范围的关键帧的 mnBAFixedForKF 标记为 pKF（触发局部 BA 的当前关键帧）的 mnId
     ...
     */
    // todo 4 作用：闭环时本质图优化，仅优化所有关键帧位姿，不优化地图点
                                           ...
                                           const map<KeyFrame *, set<KeyFrame *>> &LoopConnections, const bool &bFixScale)
    {
        ...
        g2o::BlockSolver_7_3::LinearSolverType *linearSolver =
            new g2o::LinearSolverEigen<g2o::BlockSolver_7_3::PoseMatrixType>(); // ? 有点看不懂这行代码

        // 创建块求解器
        g2o::BlockSolver_7_3 *solver_ptr = new g2o::BlockSolver_7_3(linearSolver);

        // 创建总求解器（Solver），使用 LM 算法进行非线性迭代
        g2o::OptimizationAlgorithmLevenberg *solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
     ...
     */
    // todo 3 作用：闭环时对固定地图点进行 Sim(3) 优化
    int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
    {
        ...
        // 使用 dense 的求解器，（常见非 dense 求解器有 cholmod 线性求解器和 shur 补线性求解器）
        ...
        // Sim3 需要优化
        vSim3->setFixed(false);                           // 因为要优化 Sim3 顶点，所以设置为 false
        ...
        vector<g2o::EdgeSim3ProjectXYZ *> vpEdges12;        // pKF2 对应的地图点到 pKF1 的投影边
        vector<g2o::EdgeInverseSim3ProjectXYZ *> vpEdges21; // pKF1 对应的地图点到 pKF2 的投影边
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (pMP1 && pMP2)
            {
                ...
                {
                    // 如果这对匹配点都靠谱，并且对应的 2D 特征点也都存在的话，添加 PointXYZ 顶点
            ...
            // Step 4.1 从闭环候选帧的地图点投影到当前关键帧的边 -- 【正向投影】
            g2o::EdgeSim3ProjectXYZ *e12 = new g2o::EdgeSim3ProjectXYZ();

            // vertex(id2)对应的是 pKF2 VertexSBAPointXYZ 类型的三维点
            e12->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id2)));
            // ? 为什么这里添加的节点的id为0？
            // 回答：因为 vertex(0) 对应的是 VertexSim3Expmap 类型的待优化 Sim3，其id 为 0
            ...
            // vertex(id1) 对应的是 pKF1 VertexSBAPointXYZ 类型的三维点，内部误差公式也不同
        ...
        // 如果经过上面的剔除后剩下的匹配关系已经非常少了,那么就放弃优化。内点数直接设置为 0
        if (nCorrespondences - nBad < 10)
            return 0;

        // Optimize again only with inliers
        // Step 7：再次 g2o 优化剔除后剩下的边
        optimizer.initializeOptimization();
