...
namespace ORB_SLAM2
{
     ...
     * @return int                          成功匹配的数目
     */
    // NOTE 用于局部地图点跟踪，用所有局部地图点通过投影进行特征点匹配
    // todo 第 1 个投影匹配
    ...
    // TODO （保护成员）根据观察的视角来计算匹配的时的搜索窗口大小
     ...
     * @return                   成功匹配的数量
     */
    // NOTE 通过词袋搜索匹配，用于刚刚初始化后跟踪参考关键帧中的快速匹配
    // todo 8.2 节
     ...
     */
    // NOTE 用于闭环线程，将闭环关键帧及其共视关键帧的所有地图点投影到当前关键帧中进行投影匹配
    // todo 第 2 个投影匹配
     ...
     * @return                    成功匹配的数量
     */
    // NOTE 通过词袋搜索匹配，用于闭环 Sim(3) 时当前关键帧和闭环候选关键帧之间快速匹配
     ...
     * @return              成功匹配的数量
     */
    // NOTE 通过词袋搜索匹配，用于局部建图线程中两两关键帧之间
     ...
     * @return int              更新地图点的数量
     */
    // NOTE 用于局部建图线程中，将地图点投影到关键帧中进行匹配和融合
     ...
     */
    // NOTE 用于闭环线程，将当前关键帧闭环匹配上的关键帧及其共视关键帧组成的地图点投影到当前关键帧中，进行匹配和融合
    // todo 9.1.5 节
     ...
     */
    // NOTE 在闭环检测线程中，用 Sim(3) 变换对当前关键帧和候选闭环关键帧进行相互投影匹配，以生成更多的匹配点对
    // todo 8.4 节
    ...
    {
        // Step 1： 准备工作：内参，计算 Sim3 的逆
        const float &fx = pKF1->fx;
        const float &fy = pKF1->fy;
        const float &cx = pKF1->cx;
        const float &cy = pKF1->cy;

        // 从 world 到 camera1 的变换
        // Camera 1 from world
        cv::Mat R1w = pKF1->GetRotation();
        cv::Mat t1w = pKF1->GetTranslation();

        // 从 world 到 camera2 的变换
        // Camera 2 from world
        cv::Mat R2w = pKF2->GetRotation();
        cv::Mat t2w = pKF2->GetTranslation();
        ...
        // 记录 pKF1，pKF2 中已经匹配的特征点，已经匹配记为 true，否则 false
        ...
        for (int i = 0; i < N1; i++)
        {
            ...
            if (pMP)
            {
                // pKF1 中第 i 个特征点已经匹配成功
                vbAlreadyMatched1[i] = true;
                // 得到该地图点在关键帧 pkF2 中的 id
                int idx2 = pMP->GetIndexInKeyFrame(pKF2);
                if (idx2 >= 0 && idx2 < N2)
                    // pKF2 中第 idx2 个特征点在 pKF1 中有匹配
        ...
        for (int i1 = 0; i1 < N1; i1++)
        {
            ...
            // Step 3.1：通过 Sim(3) 变换，将 pKF1 的地图点投影到 pKF2 中的图像坐标
            cv::Mat p3Dw = pMP->GetWorldPos();
            // 把 pKF1 的地图点从 world 坐标系变换到 camera1 坐标系
            cv::Mat p3Dc1 = R1w * p3Dw + t1w;
            // 再通过 Sim(3) 将该地图点从 camera1 变换到 camera2 坐标系
        ...
        // Step 5： 一致性检验,只有在两次互相匹配中都出现才能够认为是可靠的匹配
        // Check agreement
        int nFound = 0;
        for (int i1 = 0; i1 < N1; i1++)
        {
     ...
     * @return int                      成功匹配的数量
     */
    // NOTE 用于恒速模型跟踪，用前一个普通帧投影到当前帧中进行匹配
    // todo 第 3 个投影匹配 --- 最复杂的一个！8.3节
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        // 根据相对平移 Z 分量，来判断相机前进还是后退
        // 仅针对双目相机或 RGB-D 相机：如果 Z 大于基线，则表示相机明显前进
        const bool bForward = tlc.at<float>(2) > CurrentFrame.mb && !bMono;
        // 仅针对双目相机或 RGB-D 相机：如果 -Z 小于基线，则表示相机明显后退
        ...
        for (int i = 0; i < LastFrame.N; i++)
        {
            ...
            if (pMP)
            {
                ...
                {
                    // 根据位姿将上一帧有效的 MapPoints 投影到当前帧坐标系（变换到当前相机坐标系下）
                    ...
                    // 搜索窗口大小和特征点所在的金字塔尺度有关，尺度越大，搜索范围越广
                    // 窗口扩大系数为 th，单目：th = 7，双目：th = 15
                    ...
                    // Step 4 根据相机的前后前进方向来判断搜索尺度范围。
                    // 以下可以这么理解，例如一个有一定面积的圆点，在某个尺度 n 下它是一个特征点
                    // 当相机前进时，圆点的面积增大，在某个尺度 m 下它是一个特征点，由于面积增大，则原来的特征点需要在更高的尺度下才能找到正确的匹配点
                    // 当相机后退时，圆点的面积减小，在某个尺度 m 下它是一个特征点，由于面积减小，则原来的特征点需要在更低的尺度下才能找到正确的匹配点
                    if (bForward) // 前进,则当前特征点所在的金字塔尺度  nCurOctave >= nLastOctave
                    {
                        vIndices2 = CurrentFrame.GetFeaturesInArea(u, v, radius, nLastOctave);
                    }
                    else if (bBackward) // 后退,则当前特征点所在的金字塔尺度 0 <= nCurOctave <= nLastOctave
                    {
                        vIndices2 = CurrentFrame.GetFeaturesInArea(u, v, radius, 0, nLastOctave);
                    }
                    else // 在没有明显的前进或者后退及单目相机的情况下，当前特征点所在的金字塔尺度为 [nLastOctave-1, nLastOctave+1]
                    {
                    ...
                    // Step 5 遍历候选匹配点，寻找描述子距离最小的特征点作为最佳匹配点
        ...
        // Step 7 对所有匹配特征点进行旋转一致性检验，剔除不一致的匹配
     ...
     * @return int                      成功匹配的数量
     */
    // NOTE 用于重定位跟踪，将候选关键帧中未匹配的地图点投影到当前帧中，生成新的匹配
    // todo 第 4 个投影匹配
