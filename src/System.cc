...
// 包含了一些自建库
#include "System.h"
#include "Converter.h" // TODO 目前还不是很明白这个是做什么的——我知道，因为后续添加了Converter.h中的类别，所以要加上
// 包含共有库
#include <thread>              //多线程
...
namespace ORB_SLAM2
{
                                            ...
                                            mbDeactivateLocalizationMode(false)    // 没有这个模式转换标志
    {
        ...
        // Check settings file
        // new1 在构造函数函数中对mySettingFile成员变量赋值
        ...
        mpTracker = new Tracking(this,               // TODO 现在还不是很明白为什么这里还需要一个 this 指针
                                 mpVocabulary,       // 字典
                                 mpFrameDrawer,      // 帧绘制器
                                 mpMapDrawer,        // 地图绘制器
                                 mpMap,              // 地图
        ...
        mpLocalMapper = new LocalMapping(mpMap,                 // 指定使 iomanip
                                         mSensor == MONOCULAR); // TODO 为什么这个要设置成为 MONOCULAR？？？
        // NOTE 运行这个局部建图线程
    ...
    // todo2 当输入图像 为RGBD时进行的追踪，参数就不在一一说明了------追踪深度相机数据，返回mpTracker->GrabImageRGBD(im,depthmap,timestamp)
    ...
    // todo3 同理，输入为单目图像时的追踪器接口------追踪单目相机数据返回mpTracker->GrabImageMonocular(im,timestamp)
    ...
    // 激活定位模式------mbActivateLocalizationMode = true，激活   mbActivateLocalizationMode
    ...
    // 取消定位模式------mbDeactivateLocalizationMode = true，激活  mbDeactivateLocalizationMode
    ...
    // 按照 TUM 格式保存相机运行轨迹并保存到指定的文件中------求数据集中每一帧的位姿
    ...
    // 按照 KITTI 数据集的格式将相机的运动轨迹保存到文件中------求数据集中每一帧的位姿
