...
namespace DBoW2
{
  ...
  // todo 功能：将一幅图像里所有特征点，转化为两个 std::map 容器 BowVector 和 FeatureVector
  /**
  ...
  void TemplatedVocabulary<TDescriptor, F>::transform(const std::vector<TDescriptor> &features, BowVector &v, FeatureVector &fv, int levelsup) const
  {
    ...
    if (m_weighting == TF || m_weighting == TF_IDF)
    {
      ...
      for (fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
      {
        ...
        //  将当前描述子转化为 Word id， Word weight，节点所属的父节点 id（这里的父节点不是叶子的上一层，它距离叶子深度为 levelsup）
        // w is the idf value if TF_IDF, 1 if TF
        transform(*fit, id, w, &nid, levelsup);

        if (w > 0) // not stopped
        {
          // 如果 Word 权重大于 0，将其添加到 BowVector 和 FeatureVector
          v.addWeight(id, w);
  ...
  // todo 功能：一个描述子转化为Word id， Word weight，节点所属的父节点（距离叶子深度为level up深度的节点）id
  /**
  ...
  bool TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(const std::string &filename)
  {
    ...
    //! 但是没有影响，因为这里只是 reserve（预留空间），实际存储是一步步 resize 实现
    ...
    while (!f.eof())
    {
      ...
      // nid 表示当前节点 id，实际是读取顺序，从 0 开始
      int nid = m_nodes.size();
      // 节点size 加 1
      m_nodes.resize(m_nodes.size() + 1);
      m_nodes[nid].id = nid;

      // 读每行的第 1 个数字，表示父节点 id
      int pid;
      ssnode >> pid;
      // 记录节点 id 的相互父子关系
      ...
      // 读取第 2 个数字，表示是否是叶子（Word）
      int nIsLeaf;
      ssnode >> nIsLeaf;

      // 每个特征点描述子是 256 bit，一个字节对应 8 bit，所以一个特征点需要 32 个字节存储。
      // 这里 F::L=32，也就是读取 32 个字节，最后以字符串形式存储在 ssd 中
      stringstream ssd;
      ...
      if (nIsLeaf > 0)
      {
        // 如果是叶子（Word），存储到 m_words 中
        int wid = m_words.size();
        m_words.resize(wid + 1);

        // 存储 Word 的 id，具有唯一性
        m_nodes[nid].word_id = wid;
        // 构建 vector<Node*> m_words，存储 word 所在 node 的指针
  ...
  void TemplatedVocabulary<TDescriptor, F>::saveToTextFile(const std::string &filename) const
  {
    ...
    // 第一行打印 字典树的分支数、深度、评分方式、权重计算方式
    f << m_k << " " << m_L << " " << " " << m_scoring << " " << m_weighting << endl;

    // 从第 2 行开始统一的格式 ： 0 0 252 188 188 242 169 109 。。。。。。
    // 从第 3 个数字开始，存储的是 256 位的描述子，实际运行时会转化为二进制数字。
    ...
    for (size_t i = 1; i < m_nodes.size(); i++)
    {
      ...
      // 每行第 1 个数字为父节点 id
      f << node.parent << " ";
      // 每行第 2 个数字标记是（1）否（0）为叶子（Word）
      if (node.isLeaf())
        f << 1 << " ";
      else
        f << 0 << " ";
      // 接下来存储 256 位描述子，最后存储节点权重（叶子的情况下才有非零值）
