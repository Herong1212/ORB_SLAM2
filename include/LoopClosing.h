...
namespace ORB_SLAM2
{
    ...
    public:
        ...
        typedef map<KeyFrame *,                                                      // 键
                    ...
                    Eigen::aligned_allocator<std::pair<const KeyFrame *, g2o::Sim3>> // 指定分配器,和内存空间开辟有关. 为了能够使用 Eigen 库中的 SSE 和 AVX 指令集加速,需要将传统 STL 容器中的数据进行对齐处理
                    >
            KeyFrameAndPose;
    ...
    public:
         ...
         * @param[in] pDB           词袋数据库
         * @param[in] pVoc          词典
         * @param[in] bFixScale     表示 sim3 中的尺度是否要计算,对于双目和 RGB-D 情况尺度是固定的,s=1,bFixScale=true; 而单目下尺度是不确定的,此时 bFixScale=false,sim
         * 3中的s需要被计算
         */
